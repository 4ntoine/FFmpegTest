...
public class LiveHLSRecorder extends HLSRecorder{
	...
	public LiveHLSRecorder(Context c, Settings settings, int videoStreamsCount, boolean audioStream){
		super(c, settings, videoStreamsCount, audioStream);

		s3Client = new S3Client(c, "", "");
	...
	private HLSCallback externalCallback;

	public HLSCallback getExternalCallback() {
		return externalCallback;
	}

	public void setExternalCallback(HLSCallback externalCallback) {
...
//				});
				...
				if (externalCallback != null)
					externalCallback.onSegmentComplete(path);
			}

			@Override
			public void onManifestUpdated(String path) {
				if (VERBOSE)
					Log.i(TAG, ".m3u8 updated: " + path);
				...
				*/
				...
				if (externalCallback != null)
					externalCallback.onManifestUpdated(path);
